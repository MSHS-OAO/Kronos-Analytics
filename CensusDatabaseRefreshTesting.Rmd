...
if(nrow(census_summary) > 0) {
  ...
  tryCatch({
    print("Census Table - Before first truncate of temporary census table")
    dbExecute(oao_personal_conn, truncate_query)
    print("Census Table - After first truncate of temporary census table")
    dbExecute(oao_personal_conn, all_data)
    print("Census Table - After all data glue statement")
    dbExecute(oao_personal_conn, query)
    print("Census Table - After merge into census repo table")
    dbExecute(oao_personal_conn, truncate_query)
    print("Census Table - After second truncate of temporary census table")
    dbCommit(oao_personal_conn)
    dbDisconnect(oao_personal_conn)
    print("Census Table - Successfully updated!")
  }, 
  error = function(err){
    #print(err)
...
print(ifelse(target_update, "This is a refresh time", "This is not a refresh time."))
```
...
# target update query
  ...
  print("Success - Target table created.")
```
...
dbBegin(oao_personal_conn)
  ...
  tryCatch({
    print("Target Table - Before first truncate")
    dbExecute(oao_personal_conn, truncate_target)
    print("Target Table - After first truncate")
    dbExecute(oao_personal_conn, all_data_target)
    print("Target Table - After all data glue statement")
    dbExecute(oao_personal_conn, merge_target)
    print("Target Table - After merge")
    dbExecute(oao_personal_conn, truncate_target)
    print("Target Table - After second truncate")
    dbCommit(oao_personal_conn)
    dbDisconnect(oao_personal_conn)
    print("Target Table - Successfully Updated!")
  },
  error = function(err){
    dbRollback(oao_personal_conn)
    dbExecute(oao_personal_conn, truncate_query)
